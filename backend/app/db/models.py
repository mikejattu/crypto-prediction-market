from sqlalchemy import Column, String, Boolean, TIMESTAMP, Text, ForeignKey, DECIMAL, func
...
class Platform(Base):
    ...
    markets = relationship("Market", back_populates="platform", cascade="all, delete-orphan")
...
class Market(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    platform_id = Column(UUID(as_uuid=True), ForeignKey("platforms.id"), nullable=False, index=True)
    crypto_category_id = Column(UUID(as_uuid=True), ForeignKey("crypto_categories.id"), index=True)
    ...
    contracts = relationship("Contract", back_populates="market", cascade="all, delete-orphan", foreign_keys="Contract.market_id")
...
class Contract(Base):
    ...
    market = relationship("Market", back_populates="contracts", foreign_keys=[market_id])
